...
import StandardLogger from './logging/standard_logger';
...
import { isStandardLoggingDisabled, getLogger } from './logging';
...
const RpcFramework: RpcFrameworkInterface = {
  ...
  */
  initLogger: (options) => {
    const service_id = (_.isObject(options) && options.service_id) ? options.service_id : options;
    const loggerType = (!isStandardLoggingDisabled(service_id)) ? StandardLogger : LoggerHelper; 
  ...
  * How to use –
  *   import { getSingleton } from 'openapi-rpc-node';
